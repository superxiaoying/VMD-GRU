...
if __name__ == '__main__':
    ...
    #ts_df = ts_df.drop('20200706',axis=1)
    gt_df = pd.read_csv('./data/rec_trend.csv')
    gt_df = gt_df.to_numpy().reshape(-1,1)
    gps_df = pd.read_csv('./data/gps.csv')
    gps_df = gps_df.to_numpy().reshape(-1,1)
    ...
    #plt.plot(time_stamp, X_noise[0, mask_idx], 'k.--', linewidth=plt_linewidth, markersize=plt_markersize, label="Input TS")
    #plt.plot(time_stamp, data_trend[0, :]-data_trend[0, :][0], 'm.-', linewidth=plt_linewidth, markersize=plt_markersize, label="Trend Est.")
    #plt.plot(time_stamp, gt_df, 'b.-', linewidth=plt_linewidth, markersize=plt_markersize, label="Ground Truth")
    ax.plot(date_list, X_noise[0, mask_idx], 'r.--', linewidth=plt_linewidth, markersize=plt_markersize, label="TS-InSAR")
    #plt.plot(date_list_full, 1.5*data_seasonal[0, :] + np.mean(data_trend), 'b.-', linewidth=plt_linewidth, markersize=plt_markersize, label="Seasonal Est.")
    # plt.plot(date_list_full, data_seasonal[0, :], 'b.-', linewidth=plt_linewidth, markersize=plt_markersize, label="Seasonal Est.")
    ax.plot(date_list_full, data_trend[0, :]-data_trend[0, :][0], 'm.--', linewidth=plt_linewidth, markersize=plt_markersize, label="GRU baseline")
    #plt.plot(date_list_full, gt_df, color='black',linestyle='-', linewidth=plt_linewidth+1, markersize=plt_markersize, label="VMD-GRU")
    ax.plot(date_list_full, gps_df, color='green',linestyle='-', linewidth=plt_linewidth, markersize=plt_markersize, label="GNSS")
    #plt.plot(date_list_full, data_trend[0, :], 'm.-', linewidth=plt_linewidth, markersize=plt_markersize, label="Trend Est.")
    ...
    plt.title('Time series of the deformation', fontsize=18, fontweight='bold')
    plt.xticks(fontsize=12, fontweight='bold')
    plt.yticks(fontsize=12, fontweight='bold')
    plt.legend(by_label.values(), by_label.keys(), loc="upper left", fontsize=10, fancybox=True, framealpha=0.5, prop=dict(weight='bold'))
    plt.xlabel("Timestamp", fontsize=15, fontweight='bold')
    plt.ylabel("Deformation (mm)", fontsize=15, fontweight='bold')    
    ...
    ax1=ax.twinx()
    ax1.set_ylabel("Ann. avg. temp. (°C)", fontsize=15, fontweight='bold')
    temp = [0 for _ in range(92)]
    temp[16] = 24.5
    temp[46] = 24.4
    temp[76] = 24.6
    plt.plot([date_list_full[16],date_list_full[46],date_list_full[76]],[temp[16],temp[46],temp[76]], color='b', linestyle='--', linewidth=plt_linewidth, marker= 'o', markersize=plt_markersize, label="Temperature")
    ax1.set_ylim(23.8,25)
    plt.legend(loc="upper right", fontsize=10, fancybox=True, framealpha=0.5, prop=dict(weight='bold'))  
