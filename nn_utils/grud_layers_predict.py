...
class GRUDCell(GRUCell):
    ...
    def build(self, input_shape):
        ...
        if self.input_decay is not None:
            self.input_decay_kernel = self.add_weight(
                shape=(self.units, input_dim), # change for input prediction
            ...
            if self.use_decay_bias:
                self.input_decay_bias = self.add_weight(
                    shape=(input_dim,), # change for input prediction
    ...
    def call(self, inputs, states, training=None):
        ...
        input_d = K.abs(input_s - s_prev_tm1) # non-negative for backword iteration modified by zyzhao
        ...
        if self.input_decay is not None:
            ...
            x_predict = K.dot(h_tm1, self.input_decay_kernel)
            if self.use_decay_bias:
                # gamma_di = K.bias_add(gamma_di, self.input_decay_bias)
                x_predict = K.bias_add(x_predict, self.input_decay_bias)
            # gamma_di = self.input_decay(gamma_di) # no decay for prediction
        if self.hidden_decay is not None:
        ...
        if self.input_decay is not None:
            ...
            x_t = K.switch(input_m, input_x, x_predict)
...
class GRUD(GRU):
    ...
    def __call__(self, inputs, initial_state=None, **kwargs):
        ...
        print('__call__ initial_state:', initial_state)
...
class Bidirectional_for_GRUD(Bidirectional):
    def __call__(self, inputs, initial_state=None, constants=None, **kwargs):
        ...
        # print('BiGRUD__call__ initial_state:', initial_state)
